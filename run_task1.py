...
#     sp_weights = [1,1,1]
    ...
    add_no_re = False
    logger.info("stars}adding noise in collect_func") if add_no_re else None
...
def main():
    ...
    if model_args.encoder_decoder_mode:
        ...
        # state_dict = torch.load("/data2_from_58175/wav2vec2_output/filteredbyctc_continue/ft10h-checkpoint-950-8.36-0.07537-0.0754-0.0740/pytorch_model.bin",map_location="cpu")
        state_dict = torch.load("/data2_from_58175/wav2vec2_output/filteredbyctc_continue/ft1h-checkpoint-289-8.865-0.08016-0.07905/pytorch_model.bin",map_location="cpu")
        # pseudo_state_dict = torch.load("/data2_from_58175/wav2vec2_output/filteredbyctc_continue/ft10h-checkpoint-950-8.36-0.07537/pytorch_model.bin",map_location="cpu")
        model.load_state_dict(state_dict)   
        # /data2_from_58175/wav2vec2_output/filteredbyctc_continue/ft10h-checkpoint-950-8.36-0.07537
        # /home/wav2vec2_output/fisher_swbd_ft_1h_lsm_wd3_ctc5/-checkpoint-420
        # model.encoder.save_pretrained("/data2_from_58175/huggingface/models/wav2vec2_gpt2/encoder_ft1h")
        pseudo_model.load_state_dict(pseudo_state_dict) if model_args.pseudo_onthefly else None    
    ...
    #     print(f"{stars}get weight of model.lm_head={model.lm_head} from pretrained gpt2{stars} ")
    ...
    pseudo_dataset_path = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h_ft1h"
    # pseudo_dataset_path = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h_ft10h"  
    # pseudo_dataset_path = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h_ft1h_semi_50"  
    # pseudo_dataset_path_0 = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h"  
    ...
    pseudo_dataset = load_from_disk(pseudo_dataset_path)
    ignored_columns = list(set(pseudo_dataset.column_names) - set(["labels","length","speech"]))
    pseudo_dataset =pseudo_dataset.remove_columns(ignored_columns)
    
    # pseudo_dataset_0 = load_from_disk(pseudo_dataset_path_0)
    # ignored_columns = list(set(pseudo_dataset_0.column_names) - set(["labels","length","speech"]))
    # pseudo_dataset_0 = pseudo_dataset_0.remove_columns(ignored_columns)

    id_json = "label_check_ft1h_id_90_.json"
    # id_json = "filtered_double_score_id.json"
    filter_percent = 0.5
    # 89h 57520
    ...
    # pseudo_dataset_filtered = pseudo_dataset
    # logger.info(f"filter_percent of pseudo_dataset_filtered:\n{filter_percent}\n")
    logger.info(f"pseudo_dataset_filtered:\n{pseudo_dataset_filtered}\n")
    # train_dataset_teacher = datasets.concatenate_datasets([train_dataset_teacher,pseudo_dataset_10h,pseudo_dataset_89h])
    
    # id_json_0 = "filtered_score_redu_id.json"
    # filter_semi_percent = 0.00002
    # with open(id_json_0,"r",encoding="utf-8") as f:
        # filtered_id_0 = json.load(f)["id"][:int(57520*filter_semi_percent)]
    # pseudo_dataset_filtered_0 = pseudo_dataset_0.select(filtered_id_0)#.select(range(7000))
    # logger.info(f"pseudo_dataset_filtered_0:\n{pseudo_dataset_filtered_0}\n")

    train_dataset = datasets.concatenate_datasets([train_dataset_teacher,pseudo_dataset_filtered])
    # train_dataset = datasets.concatenate_datasets([train_dataset,pseudo_dataset_filtered,pseudo_dataset_filtered_0])
    # train_dataset = pseudo_dataset_filtered
    logger.info(f"{stars}get dataset over{stars}")
    logger.info(time.strftime('%Y-%m-%d %H:%M:%S'))   

    train_dataset_teacher_total_dur = sum(train_dataset_teacher["length"])/3600
    train_dataset_teacher_maxlength = sorted(train_dataset_teacher["length"],reverse=True)[0]
    ...
    pseudo_total_dur = sum(pseudo_dataset_filtered["length"])/3600
    pseudo_maxlength = sorted(pseudo_dataset_filtered["length"],reverse=True)[0]
    
    logger.info(f"train_dataset_teacher:\n{train_dataset_teacher}\n")
    logger.info(f"total duration of train_dataset_teacher:\n{train_dataset_teacher_total_dur} hours\n")
    logger.info(f"maxlength of pseudo_dataset:\n{train_dataset_teacher_maxlength} s\n")
    
    logger.info(f"pseudo_dataset_filtered:\n{pseudo_dataset_filtered}\n")
    logger.info(f"total duration of pseudo_dataset_filtered:\n{pseudo_total_dur} hours\n")
    logger.info(f"maxlength of pseudo_dataset_filtered:\n{pseudo_maxlength} s\n")
