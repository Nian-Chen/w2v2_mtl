...
from beam_search_att_rescore import (
    ...
    get_kenlm_decoder,
    get_kenlm_decoder_,
    attention_rescoring_lm_,
    attention_rescoring_lm__,
)
...
def main():
    ...
    kenlm_model_path = "/tsdata/xsp/w2v2/lm_4gram_fisher.arpa"
    ...
    gpt_path = "/data2_from_58175/wav2vec2_output/gpt/vocab32-checkpoint-50640"
    ...
    def map_to_pred_wenet_att_rescore(batch):
        if "input_values"in batch.keys():
            # 优先使用input_values
            input_values = torch.tensor(batch["input_values"]).to(device)
        else:
            features = processor(batch["speech"], return_tensors="pt", padding="longest",sampling_rate=16000) 
            input_values = features.input_values.to(device)
            attention_mask = features.attention_mask.to(device)
        if "text" not in batch.keys():
        ...
        # predicted_ids,predicted_ids_bs = attention_rescoring_lm__(input_values=input_values,
                                                             # processor=processor,
                                                             # gpt_model=gpt_model,
                                                             ...
                                                             # lm_model=lm_model,
                                                             # )
        # vote_result,predicted_ids,predicted_ids_bs = attention_rescoring_lm__(input_values=input_values,
                                                             # processor=processor,
                                                             # gpt_model=gpt_model,
                                                             # gpt_tokenizer=gpt_tokenizer,
                                                             # beam_size=20,
                                                             ...
                                                             # kenlm_decoder=kenlm_decoder_,
                                                             # )
        # print(f"vote_result = {vote_result}")
        # batch["vote_result"] = [vote_result]
