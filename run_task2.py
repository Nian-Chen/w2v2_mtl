...
import os
os.environ["TRANSFORMERS_CACHE"] = "/home/huggingface/transformers"
os.environ["HF_DATASETS_CACHE"] = "/home/huggingface/datasets"
os.environ["HF_METRICS_CACHE"] = "/home/huggingface/metrics"
os.environ["HF_HOME"] = "/home/huggingface"
...
class DataCollatorCTCWithPadding_Speed_Perturb:
    ...
    pad_to_multiple_of: Optional[int] = None
    pad_to_multiple_of_labels: Optional[int] = None
    speeds = np.linspace(0.9,1.1,21).tolist()
    sp_weights = (np.ones(21)).tolist()
#     sp_weights = (np.ones(21)*(1-0.3)/20).tolist()
#     sp_weights[10] = 0.5
#     print(sp_weights)
#     speeds = [0.9,1.0,1.1]
#     sp_weights = [1,1,1]
    print("~"*20, "doing speed perturbation in collect_func", "~"*20) if len(speeds)>1 else None
    add_no_re = False
    print("~"*20, "adding noise in collect_func", "~"*20) if add_no_re else None
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        #！！！！！！！！！！！！！！！！input_values是list，但其中每个数都是float64，而sox变速只接收float32的数据，导致即使speed为1，得到的数据也有些许差异，虽然e-8级别，但不容小视
        # if speed != 1.0:
            # input_features = [{"input_values": sox_effects.apply_effects_tensor(
                        # tensor = torch.tensor(feature["input_values"]).unsqueeze(0),
                        # sample_rate = 16000,
...
#################################初始2个dict保存最后两个epoch的结果，deepcopy才能将dict完全与模型脱离开，否则模型更新时二者会同步
        if self.pseudo_model is not None:
            ...
            print("initialing state_dict_original and pseudo_model")
            state_dict_original = copy.deepcopy(self.pseudo_model.state_dict())
            # state_dict_original = torch.load("/data2_from_58175/wav2vec2_output/fisher_joint/-checkpoint-5000/pytorch_model.bin",map_location="cpu")
            # print(state_dict_original)
            self.pseudo_model.load_state_dict(state_dict_original)
            # pseudo_model = self._wrap_model(pseudo_model, training=False)
            # 设置更新节点，每10%更新一次
            onthefly_point = [int(max_steps*i) for i in np.arange(2)/2]
            print(f"we will update the pseudo_model at {onthefly_point} step，but step 0 use the original_model")
        ...
        self.decoder_teacher_loss = False
        self.decoder_entropy_loss = False
        self.encoder_entropy_loss = False
        self.encoder_teacher_loss = False
...
#################################最初点不进行更新,直接使用state_dict_original        
                if (self.pseudo_model is not None) and self.update_pseudo_model:
                    ...
                    if step% args.gradient_accumulation_steps == 0 and onthefly_update_state[self.state.global_step]==False and self.state.global_step in onthefly_point[1:]:
                        # print(step)
                        # print(f"update pseudo_model at {onthefly_point} step")
                        print(f"current step = {self.state.global_step},updating the pseudo_model")
                        state_dict = copy.deepcopy(model.module.state_dict())
                        # 同时赋值才能保证state_dict_last不会等于state_dict_last_last
                        # print(state_dict_original!=state_dict_last)
                        # assert (state_dict_original!=state_dict_last),f"state_dict_last is equal to state_dict_original"
                        # pseudo_model_type = random.choices([1,2,3], [1,0,0], k=1)[0]
                        # if pseudo_model_type == 1:
                            # print("pseudo_model = original_model")
                            # state_dict = state_dict_original
                        # elif pseudo_model_type == 2:
                            # print("pseudo_model = last_epoch_model")
                            # state_dict = state_dict_last
                        # else:
                            # print("pseudo_model = 0.5*last_epoch_model + 0.5*original_model")
                            # state_dict = state_dict_last
                            # for k in state_dict.keys():
                                # state_dict[k] = 0.5*state_dict[k] + 0.5*state_dict_original[k]
                        online_model_weight = 0.5
                        # online_model_weight = self.state.global_step/max_steps/2
                        print(f"online_model_weight = {online_model_weight}")
                        for k in state_dict.keys():
                            state_dict[k] = online_model_weight*state_dict[k] + (1-online_model_weight)*state_dict_original[k]
...
#                 set_trace()
                if self.pseudo_model is not None:
                    ...
                    # # print(f"original_labels = {inputs['labels'].shape}")
                    ...
                    # print(f"onethefly labels = {inputs['labels'].shape}")
                    # print(f"onethefly labels.shape:{labels.shape}") 
                    # print((tmp!=inputs['labels']).sum())
                # print(f"original_labels = {inputs['labels']}")
...
#########################################################
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        if ((self.pseudo_model is not None and self.decoder_teacher_loss) or self.decoder_entropy_loss or self.encoder_entropy_loss)and self.model.training:
            ...
            if self.encoder_teacher_loss and self.decoder_teacher_loss:
            ...
            elif self.decoder_entropy_loss and self.encoder_entropy_loss:
                outputs = model(**inputs,decoder_entropy_loss=True,encoder_entropy_loss=True)
            elif self.decoder_entropy_loss:
                outputs = model(**inputs,decoder_entropy_loss=True)
            elif self.decoder_teacher_loss:
...
#                         print(f"sum_att_loss={att_loss.item()}")
        ...
        if self.eval_only_encoder and freeze_decoder:
            ctc_weight = 1.0
        if model is not self.model :
...
def main():
    ...
    if model_args.encoder_decoder_mode:
        encoder_model_path = "/home/models/wav2vec2_gpt2/encoder"
        decoder_model_path = "/home/models/wav2vec2_gpt2/decoder"
        ...
        pseudo_model = Wav2vec2_Gpt2(encoder=encoder_,decoder=decoder_) if model_args.pseudo_onthefly else None
        state_dict = torch.load("/data2_from_58175/wav2vec2_output/filteredbyctc_continue/ft10h-checkpoint-950-8.36/pytorch_model.bin",map_location="cpu")
        model.load_state_dict(state_dict)   
        # model.encoder.save_pretrained("/data2_from_58175/huggingface/models/wav2vec2_gpt2/encoder_ft1h")
        # model.encoder.save_pretrained("/data2_from_58175/huggingface/models/wav2vec2_gpt2/decoder_ft1h")
        pseudo_model.load_state_dict(state_dict) if model_args.pseudo_onthefly else None    
        # print(model.encoder.lm_head.weight)
        # print(pseudo_model.encoder.lm_head.weight)
        print(f"!!!!!!!!!!!!use Wav2vec2_Gpt2 model!!!!!!!!!!!! ")
            
    # teacher_model = Wav2Vec2ForCTC.from_pretrained(
    #     model_args.model_name_or_path,
    #     cache_dir="/data2_from_58175/huggingface/models",
    #     gradient_checkpointing=model_args.gradient_checkpointing,
    #     vocab_size=32,
    ...
    if not model_args.use_gpt_tokenizer:
        ...
        print(f"!!!!!!!!!!!!use_gpt_tokenizer,vocab_size={processor.tokenizer.vocab_size}!!!!!!!!!!!! ")
    ...
    wer_metric = datasets.load_metric("/home/huggingface/metrics/wer")
    ...
    pseudo_dataset_path = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h_ft10h"  
    # print("swbd_train_12s 1h用做训练，计算test集[:1000]的wer")
    # train_dataset = load_from_disk(prepare_train_path).select(np.arange(48))
    # train_dataset = load_from_disk(prepare_train_path).select(np.arange(800))
    # 2.7h伪标签
    ...
    print("*"*20,"loading train_dataset","*"*20)
    ...
    # 去除swbd测试集和100小时fisher集,总1980h
    # train_id_json = "fisher_swbd_train_id.json"    
    # with open(train_id_json,"r",encoding="utf-8") as f:
        # train_id_list = json.load(f)["id"]
    # train_dataset = train_dataset.select(train_id_list)   

    # test_id_json = "fisher_swbd_swbdtest_5h_id.json"    
    # with open(test_id_json,"r",encoding="utf-8") as f:
        # test_id_list = json.load(f)["id"]
    val_dataset = load_from_disk(prepare_test_path).remove_columns(['sampling_rate', 'seg_end', 'seg_start', 'text'])
    ...
    # 0-6850是1h
    train_dataset = train_dataset.select(range(6850)).remove_columns(['file', 'id'])
    # train_dataset = train_dataset.select(range(6850,6850+680)).remove_columns(['file', 'id'])
    print(time.strftime('%Y-%m-%d %H:%M:%S'))
    print("*"*20,"loading pseudo_dataset","*"*20)
    pseudo_dataset = load_from_disk(pseudo_dataset_path).remove_columns(['score', 'wer', "text"])
    # pseudo_dataset_10h = pseudo_dataset.select(range(6850))
    # pseudo_dataset_89h = pseudo_dataset.select(range(6850+680,65050))
    # 只要['labels', 'length', 'speech']
    id_json = "filtered_ft10h_ctc_id.json"
    filter_percent = 0.9
    # 89h 57520
    with open(id_json,"r",encoding="utf-8") as f:
        filtered_id = json.load(f)["id"][:int(57520*filter_percent)]
    pseudo_dataset_filtered = pseudo_dataset.select(filtered_id)#.select(range(7000))
    # train_dataset = datasets.concatenate_datasets([train_dataset,pseudo_dataset_10h,pseudo_dataset_89h])
    train_dataset = datasets.concatenate_datasets([train_dataset,pseudo_dataset_filtered])
    print("*" * 20, "get dataset over", "*" * 20)
    print(time.strftime('%Y-%m-%d %H:%M:%S'))   
    ...
    pseudo_total_dur = sum(pseudo_dataset["length"])/3600
    pseudo_maxlength = sorted(pseudo_dataset["length"],reverse=True)[0]
    print(f"pseudo_dataset:\n{pseudo_dataset}\n")
    print(f"total duration of pseudo_dataset:\n{pseudo_total_dur} hours\n")
    ...
    # prepare_train_path = "/work_from_58175/fisher_swbd_nodup_onlyspeech/ted/train_100h"  
    # prepare_validation_path = "/work_from_58175/fisher_swbd_nodup_onlyspeech/ted/test_2h"

    # print(time.strftime('%Y-%m-%d %H:%M:%S'))
    # print("*"*20,"loading train_dataset","*"*20)
    # train_dataset = load_from_disk(prepare_train_path).remove_columns(['sampling_rate', 'seg_end', 'seg_start', 'text'])
    # # 当speech和inputvalues都存在时，除去input_values
    # print("~" * 20, "get train_dataset over", "~" * 20)
    # print(time.strftime('%Y-%m-%d %H:%M:%S'))
    # if ("speech" in train_dataset.features) and ("input_values" in train_dataset.features):
        # train_dataset = train_dataset.remove_columns(["input_values"])  
    # else:
    ...
    # val_dataset = load_from_disk(prepare_validation_path).select(range(500))
    # if ("speech" in val_dataset.features) and ("input_values" in val_dataset.features):
        # val_dataset = val_dataset.remove_columns(["input_values"])  
    # else:
        # val_dataset = val_dataset       
    ...
    print("!!!!!!!!!!!!do speech perpturbation!!!!!!!!!!!!") if data_args.speed_perturb else None
    ...
    if model_args.freeze_feature_extractor:
        print("!!!!!!!!!!!!freeze parameters of freeze_feature_extractor!!!!!!!!!!!! ")
    ...
    if model_args.freeze_ALN:
        print("!!!!!!!!!!!!freeze parameters of freeze_feature_extractor and feed_forward!!!!!!!!!!!! ")
        model.freeze_ALN() if not model_args.encoder_decoder_mode else model.encoder.freeze_ALN()
    if model_args.freeze_all_except_lm:
        print("!!!!!!!!!!!!freeze all parameters of the model except lm_head!!!!!!!!!!!! ")
    ...
    if model_args.freeze_all_except_feature_extractor:
        print("!!!!!!!!!!!!freeze all parameters of the model except feature_extractor!!!!!!!!!!!! ")
    ...
    if model_args.freeze_decoder:
        print("!!!!!!!!!!!!freeze all parameters of the decoder when in encoder_decoder_mode!!!!!!!!!!!! ")
        if model_args.encoder_decoder_mode:
