...
class ModelArguments:
    ...
    freeze_ALN: Optional[bool] = field(
        default=False, metadata={"help": "Whether to freeze freeze parameters of freeze_feature_extractor and feed_forward."}
    )    
    ...
    remove_silence: Optional[bool] = field(
        default=False, metadata={"help": "Whether to exclude samples with empty transcription,usually in the end of training"}
    )
...
#                     temp_labels = inputs["labels"]
                    inputs["labels"] = pad_sequence([torch.from_numpy(np.array(i)) for i in labels],batch_first=True,padding_value=-100)
...
#             print(f"distill_weight*distill_loss={distill_weight*distill_loss}")
        ...
        return (loss, outputs) if return_outputs else loss     
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], teacher_model) -> torch.Tensor:
        """
...
def main():
    ...
    if model_args.encoder_decoder_mode:
        encoder_model_path = "/data2_from_58175/huggingface/models/wav2vec-large-xlsr-53-init-minanyu-vocabs"
        decoder_model_path = "/data2_from_58175/huggingface/models/wav2vec2_gpt2/minnanyu_decoder"
        encoder = Wav2Vec2ForCTC.from_pretrained(encoder_model_path)
        if model_args.remove_silence:
            print(f"!!!!!!!!!!!!exclude samples with empty transcriptions!!!!!!!!!!!! ")
            encoder = Wav2Vec2ForCTC_Remove_Silence.from_pretrained(encoder_model_path)
        decoder = AutoModelForCausalLM.from_pretrained(decoder_model_path)
        model = Wav2vec2_Gpt2(encoder=encoder,decoder=decoder)
        state_dict = torch.load("/data2_from_58175/wav2vec2_output/fisher_joint/minnanyu-decoder-randintcheckpoint-33000-38.18-43.20/pytorch_model.bin",map_location="cpu")
        model.load_state_dict(state_dict)
        logger.info(f"{stars}use Wav2vec2_Gpt2 model{stars} ")

    elif model_args.remove_silence:
        model = Wav2Vec2ForCTC_Remove_Silence.from_pretrained(model_args.model_name_or_path)

    else:
        encoder_config = AutoConfig.from_pretrained("/data2_from_58175/huggingface/models/wav2vec-large-xlsr-53-init-minanyu-vocabs/config_small.json")
    ...
    prepare_test_path = "/data2_from_58175/huggingface/datasets/minnanyu/dev"
