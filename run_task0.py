...
########################
            self.wer_decreased = True if metrics["wer"] < self.last_wer else False
            self.last_wer = metrics["wer"]
...
#################################初始2个dict保存最后两个epoch的结果，deepcopy才能将dict完全与模型脱离开，否则模型更新时二者会同步
        if self.pseudo_model is not None:
            ...
            # 设置更新节点，每10%更新一次
            onthefly_point = [int(max_steps*i) for i in np.arange(21)/20]
            print(f"we will update the pseudo_model at {onthefly_point[1:]} step")
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            self.update_pseudo_model = True if self.wer_decreased == True else False
            # if epoch==0:
...
#################################最初点不进行更新,直接使用state_dict_original        
                if (self.pseudo_model is not None) and self.update_pseudo_model:
                    ...
                    if step% args.gradient_accumulation_steps == 0 and  (self.state.global_step in onthefly_point[1:]):
...
#                 set_trace()
                if self.pseudo_model is not None and (self.state.global_step >= onthefly_point[1]):
                    self.pseudo_model.eval()
                    ...
                    # if self.state.global_step in onthefly_point[1:]:
...
def main():
    ...
    if model_args.encoder_decoder_mode:
        ...
        # state_dict = torch.load("/data2_from_58175/wav2vec2_output/filteredbyctc_continue/1-checkpoint-960/pytorch_model.bin",map_location="cpu")
    ...
    pseudo_dataset_path = "/home/data/fisher_swbd_nodup_onlyspeech/swbd_pseudo_89h"  
    ...
    train_dataset = pseudo_dataset_filtered
